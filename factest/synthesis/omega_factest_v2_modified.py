...
class buchi_from_ltl:
    ...
    def getAllTransitions(self):
        ...
        # print('transitions in', self.transitions_in_dict)
...
class omega_FACTEST(buchi_from_ltl):
    def __init__(self, ltl_formula, env, model = None, workspace = None, seg_max = 6, part_max = 1, shrinking_constant = 0.1, max_shrinking_depth = 5, print_statements = False) -> None:
    ...
    def exampleRun(self, num_cycles = 2):
        if len(self.buchi_inits) > 1:
            print("Too many possible initial states!") #TODO: NEED TO UPDATE THIS PART
            return None
        else:
            print(self.buchi_run)
            sample_run = []

            for state in self.buchi_run['prefix']:
                sample_run.append(state[0])

            for i in range(num_cycles):
                for state in self.buchi_run['cycle']:
                    sample_run.append(state[0])
            
            # init_states = self.transitions_out_dict[self.buchi_inits[0]]
            # print(init_states)
