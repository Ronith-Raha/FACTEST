import sys
sys.path.append('..')
from ODEs.newcars import Car_simulate
import numpy as np
...
lower = np.array([-10, -10, -np.pi/4, 0, 0.0])
higher = np.array([10, 10, np.pi/4, 2, 0.0])
...
def sample_mode():
    modes1 =['Const','Acc','Dec','Brk','TurnLeft','TurnRight']
    return random.choice(modes1)
...
def simulate(x0):
    return np.array(Car_simulate(sample_mode(), x0, TMAX))
